...
export function LandingDemoInputPanel({
  ...
  return (
    <form
      ...
      style={style}
      onSubmit={onSubmit}
      className="pointer-events-auto absolute bottom-3 left-3 z-20 flex h-[165px] flex-col overflow-hidden rounded-lg border border-zinc-700 bg-zinc-800 text-white shadow-xl md:bottom-4 md:left-4 md:h-[var(--demo-panel-height)]"
      data-tour="input-panel"
    >
      <div
        {...dragHandleProps}
        className="flex h-7 shrink-0 touch-none select-none items-center border-b border-zinc-600 bg-zinc-700 px-2 text-zinc-400 md:cursor-grab md:active:cursor-grabbing"
      >
      ...
      <textarea
        ...
        maxLength={600}
        className="scrollbar-hidden flex-1 resize-none bg-transparent p-3 text-sm leading-5 text-white outline-none placeholder:text-zinc-500 disabled:opacity-60"
      />

      <div className="flex shrink-0 justify-end border-t border-zinc-700 p-2">
        <button
          type="submit"
          disabled={!canSubmit}
          className="rounded-md bg-blue-600 px-4 py-1.5 text-sm font-medium text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-zinc-600 disabled:text-zinc-400"
        >
