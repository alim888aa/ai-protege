...
export default function PricingPage() {
  return (
    <div className="min-h-screen overflow-x-clip bg-zinc-50 text-zinc-950 dark:bg-[#09090b] dark:text-white">
      <section className="relative overflow-hidden border-b border-zinc-200 dark:border-white/10">
        <div
          className="pointer-events-none absolute inset-0 opacity-[0.11]"
          style={{
            backgroundImage:
              'linear-gradient(color-mix(in srgb, currentColor 12%, transparent) 1px, transparent 1px), linear-gradient(90deg, color-mix(in srgb, currentColor 12%, transparent) 1px, transparent 1px)',
        ...
        <div className="relative">
          ...
          <div className="mx-auto grid w-full max-w-[1440px] gap-12 px-5 pb-20 pt-16 md:px-8 md:pb-28 md:pt-24 lg:grid-cols-[1fr_22rem] lg:items-end lg:px-10">
            <div>
              <p className="mb-5 flex items-center gap-3 text-xs font-bold tracking-[0.2em] text-violet-700 dark:text-violet-300">
                <span className="h-px w-9 bg-violet-600 dark:bg-violet-300" />
                MEMBERSHIP
              </p>
              <h1 className="max-w-5xl text-[clamp(3.4rem,7.8vw,7.9rem)] font-semibold leading-[0.88] tracking-[-0.07em]">
                One membership.
                <span className="block text-violet-700 dark:text-violet-300">Two rhythms.</span>
              </h1>
              <p className="mt-8 max-w-2xl text-lg leading-8 text-zinc-600 md:text-xl dark:text-zinc-400">
                Unlimited practice for people who learn by explaining. Choose how often you want to think about billing.
              </p>
            </div>

            <aside className="border-l border-violet-500/50 pl-6 text-sm text-zinc-700 lg:mb-2 dark:border-violet-300/50 dark:text-zinc-300">
              <BrainCircuit className="mb-5 size-8 text-violet-700 dark:text-violet-300" />
              <p className="font-mono text-xs tracking-[0.16em] text-zinc-500">THE WHOLE LOOP</p>
              <p className="mt-3 text-lg leading-7 text-zinc-950 dark:text-white">Source → explain → get challenged → understand.</p>
              <a href="#plans" className="mt-7 inline-flex items-center gap-2 font-semibold text-violet-700 hover:text-violet-800 dark:text-violet-300 dark:hover:text-violet-200">
      ...
      <main id="plans" className="mx-auto w-full max-w-[1440px] px-5 py-20 md:px-8 md:py-28 lg:px-10">
        <div className="mb-10 flex flex-col gap-5 border-b border-zinc-200 pb-8 sm:flex-row sm:items-end sm:justify-between dark:border-white/10">
          <div>
          ...
          <div className="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400">
            <InfinityIcon className="size-5 text-violet-700 dark:text-violet-300" />
        ...
        <section className="mt-28 border-t border-zinc-200 pt-10 dark:border-white/10">
          ...
          <div className="grid gap-px overflow-hidden rounded-[1.75rem] border border-zinc-200 bg-zinc-200 md:grid-cols-3 dark:border-white/10 dark:bg-white/10">
            {timeline.map((item) => (
              <article key={item.label} className="bg-white p-7 sm:p-8 dark:bg-zinc-950">
                <p className="font-mono text-xs font-bold tracking-[0.16em] text-violet-700 dark:text-violet-300">{item.label.toUpperCase()}</p>
                <h3 className="mt-5 text-xl font-semibold tracking-[-0.025em]">{item.title}</h3>
                <p className="mt-3 text-sm leading-6 text-zinc-600 dark:text-zinc-400">{item.copy}</p>
              </article>
            ))}
          </div>
